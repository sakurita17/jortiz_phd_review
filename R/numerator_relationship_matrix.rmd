...
$A$ describes the expected genetic relationship between individuals before observing any phenotypic data, it serves as the prior covariance matrix of additive genetic values.Thus, The additive relationship $a_{ij}$ is used as a measure of the covariance of breeding values between relative.
...
The diagonals, the NRM contains the coefficient of inbreeding. That is the probability that two genes at any locus in an individual are identical by descent.
...
```{r}
ped = data.frame(iid = 1:8,
                 fid = c(0, 0, 1, 1, 4, 0, 5, 7), 
                 mid = c(0, 0, 2, 0, 3, 2, 0, 6))
...
```{r}
# Crear figura de corazón
t <- seq(0, 2*pi, length.out = 1000)
x <- 16 * sin(t)^3
y <- 13 * cos(t) - 5*cos(2*t) - 2*cos(3*t) - cos(4*t)

# Dibujar el corazón
plot(x, y, type = "l", col = "red", lwd = 3, axes = FALSE, xlab = "", ylab = "", main = "")
polygon(x, y, col = "pink", border = "red")

# Agregar el texto en el centro del corazón
text(0, 0, "Te amo, príncipe", col = "darkred", cex = 2.5, font = 2)
```
